...
exports.accountHolderService = exports.AccountHolderService = void 0;
...
class AccountHolderService {
     ...
     */
    async createAccountHolder(profileId, holderType, data) {
        return await prisma_1.prisma.$transaction(async (tx) => {
            // Validate data
            await validationService_1.validationService.validateConditionalFields(data, {});
            ...
            if (data.maritalStatuses && data.maritalStatuses.length > 0) {
                await tx.accountHolderMaritalStatus.createMany({
                    data: data.maritalStatuses.map((status) => ({
                        accountHolderId: accountHolder.id,
            ...
            if (data.employmentAffiliations && data.employmentAffiliations.length > 0) {
                await tx.accountHolderEmploymentAffiliation.createMany({
                    data: data.employmentAffiliations.map((affiliation) => ({
                        accountHolderId: accountHolder.id,
            ...
            if (data.investmentKnowledge && data.investmentKnowledge.length > 0) {
                await tx.accountHolderInvestmentKnowledge.createMany({
                    data: data.investmentKnowledge.map((knowledge) => ({
                        accountHolderId: accountHolder.id,
                ...
                await tx.accountHolderFinancialInformation.create({
                    data: {
                        ...
                        annualIncomeFrom: data.financialInformation.annualIncomeFrom
                            ? new library_1.Decimal(data.financialInformation.annualIncomeFrom)
                            : null,
                        annualIncomeTo: data.financialInformation.annualIncomeTo
                            ? new library_1.Decimal(data.financialInformation.annualIncomeTo)
                            : null,
                        netWorthFrom: data.financialInformation.netWorthFrom
                            ? new library_1.Decimal(data.financialInformation.netWorthFrom)
                            : null,
                        netWorthTo: data.financialInformation.netWorthTo
                            ? new library_1.Decimal(data.financialInformation.netWorthTo)
                            : null,
                        liquidNetWorthFrom: data.financialInformation.liquidNetWorthFrom
                            ? new library_1.Decimal(data.financialInformation.liquidNetWorthFrom)
                            : null,
                        liquidNetWorthTo: data.financialInformation.liquidNetWorthTo
                            ? new library_1.Decimal(data.financialInformation.liquidNetWorthTo)
                            : null,
            ...
            if (data.governmentIdentifications && data.governmentIdentifications.length > 0) {
                await tx.governmentIdentification.createMany({
                    data: data.governmentIdentifications.map((govId) => ({
                        accountHolderId: accountHolder.id,
     ...
     */
    async getAccountHolderById(holderId) {
        const accountHolder = await prisma_1.prisma.accountHolder.findUnique({
            where: { id: holderId },
            include: {
                addresses: true,
                phones: true,
        ...
        if (!accountHolder) {
            throw new errors_1.NotFoundError("Account Holder", holderId);
        }
     ...
     */
    async updateAccountHolder(holderId, updates) {
        ...
        if (!profile) {
            throw new errors_1.ConflictError("Profile not found");
        }
        ...
        return await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (updates.addresses !== undefined) {
                ...
                if (updates.addresses.length > 0) {
                    await tx.accountHolderAddress.createMany({
                        data: updates.addresses.map((addr) => ({
            ...
            if (updates.maritalStatuses !== undefined) {
                ...
                if (updates.maritalStatuses.length > 0) {
                    await tx.accountHolderMaritalStatus.createMany({
                        data: updates.maritalStatuses.map((status) => ({
                            accountHolderId: holderId,
                            maritalStatus: status,
                        })),
                    });
            ...
            if (updates.employmentAffiliations !== undefined) {
                ...
                if (updates.employmentAffiliations.length > 0) {
                    await tx.accountHolderEmploymentAffiliation.createMany({
                        data: updates.employmentAffiliations.map((affiliation) => ({
                            accountHolderId: holderId,
                            affiliation: affiliation,
                        })),
                    });
            ...
            if (updates.investmentKnowledge !== undefined) {
                ...
                if (updates.investmentKnowledge.length > 0) {
                    await tx.accountHolderInvestmentKnowledge.createMany({
                        data: updates.investmentKnowledge.map((knowledge) => ({
                            accountHolderId: holderId,
                            investmentType: knowledge.investmentType,
                            knowledgeLevel: knowledge.knowledgeLevel,
                            sinceYear: knowledge.sinceYear,
                ...
                await tx.accountHolderFinancialInformation.upsert({
                    ...
                    create: {
                        ...
                        annualIncomeFrom: updates.financialInformation.annualIncomeFrom
                            ? new library_1.Decimal(updates.financialInformation.annualIncomeFrom)
                            : null,
                        annualIncomeTo: updates.financialInformation.annualIncomeTo
                            ? new library_1.Decimal(updates.financialInformation.annualIncomeTo)
                            : null,
                        netWorthFrom: updates.financialInformation.netWorthFrom
                            ? new library_1.Decimal(updates.financialInformation.netWorthFrom)
                            : null,
                        netWorthTo: updates.financialInformation.netWorthTo
                            ? new library_1.Decimal(updates.financialInformation.netWorthTo)
                            : null,
                        liquidNetWorthFrom: updates.financialInformation.liquidNetWorthFrom
                            ? new library_1.Decimal(updates.financialInformation.liquidNetWorthFrom)
                            : null,
                        liquidNetWorthTo: updates.financialInformation.liquidNetWorthTo
                            ? new library_1.Decimal(updates.financialInformation.liquidNetWorthTo)
                            : null,
            ...
            if (updates.governmentIdentifications !== undefined) {
                ...
                if (updates.governmentIdentifications.length > 0) {
                    await tx.governmentIdentification.createMany({
                        data: updates.governmentIdentifications.map((govId) => ({
                            accountHolderId: holderId,
                            idNumber: govId.idNumber,
                            idType: govId.idType,
                            idNumberValue: govId.idNumberValue,
     ...
     */
    async getAccountHoldersByProfile(profileId) {
        return await prisma_1.prisma.accountHolder.findMany({
            where: { profileId },
            include: {
                addresses: true,
                phones: true,
...
exports.accountHolderService = new AccountHolderService();
